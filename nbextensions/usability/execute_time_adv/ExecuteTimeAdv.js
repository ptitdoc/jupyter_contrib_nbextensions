...
define(["require", "jquery", "base/js/namespace", "base/js/events"], function (require, $, IPython, events) {
    ...
    var patchCodecellExecute = function() {
        console.log('patching codecell to trigger ExecuteCell.ExecuteTimeAdv');
        ...
        IPython.CodeCell.prototype.execute = function () {
            ...
            events.trigger('ExecuteCell.ExecuteTimeAdv');
        };
    }

    var cellMarker = function(val, color) {
    ...
    var markCell = function(cell,val,color) {
        var n=0;
        // setGutterMarker: function(line, gutterID, value)
        cell.code_mirror.setGutterMarker(n, "celltime", cellMarker(val,color));
    }
    ...
    var toggleDisplay = function() {
        ...
        if (cell instanceof IPython.CodeCell) {
            ...
            if (timing_area.length === 0 && 'execute_time' in cell.metadata) {
                timing_area = createTimingArea(cell, ce);
                timing_area.text(cell.metadata['execute_time']);
            } else if (vis) {
        ...
        var link_all=$("<a/>").text("All").click(function(){
            ...
            for (var i=0; i<ncells; i++) {
                if (cells[i] instanceof IPython.CodeCell) {
                    ...
                    var vis=timing_area.is(':visible');
                    if (timing_area.length === 0 && 'execute_time' in cells[i].metadata) {
                        timing_area = createTimingArea(cells[i],(cells[i]).element);
                        timing_area.hide();
                        timing_area.text(cells[i].metadata['execute_time']);
                    } else {
                        continue;
                    }
    ...
    var executionStartTime = function (event) {
        ...
        if (cell instanceof IPython.CodeCell) {
            ...
            cell.metadata['execute_time'] = startMsg;

            var timing_area=ce.find(".timing_area");
            if (timing_area.length === 0) {
                var timing_area = createTimingArea(cell,ce);
            }
            timing_area.text(startMsg);
        }
    };

    var createTimingArea = function (cell, cellelement) {
        var timing_area = $('<div/>').addClass('timing_area');

        timing_area.dblclick(toggleDisplay);

        cellelement.find(".input_area").css('border-radius','4px 4px 0 0');
        cellelement.find(".inner_cell").append(timing_area);

        // Set timing mark in gutter
        var gutters = cell.code_mirror.getOption('gutters');
        cell.code_mirror.setOption('gutters', [gutters, "celltime" ]);
        markCell(cell,"&#x25cf;","#FFA500");
    ...
    var executionEndTime = function(event) {
        ...
        if (cell instanceof IPython.CodeCell) {
            ...
            cell.metadata['execute_time'] += durationMsg;
    ...
    var load_ipython_extension = function() {
        ...
        events.on('ExecuteCell.ExecuteTimeAdv', executionStartTime);
        events.on('kernel_idle.Kernel', executionEndTime);

        $("head").append($("<link rel='stylesheet' href='" + require.toUrl("./ExecuteTimeAdv.css") + "' type='text/css'  />"));
        create_menu();
    };
